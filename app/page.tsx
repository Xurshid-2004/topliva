...
const TrainFuelSystem = () => {
  ...
  const handleExportErjuPdf = () => {
    ...
    // NEW: Harakat turi bo'yicha hisob-kitob (move types as rows, ERJUs as columns)
    // Structure: moveType -> { erjuName: amount }
    const moveTypeData: Record<string, Record<string, number>> = {};
    const moveTypeTotals: Record<string, number> = {};
    
    // NEW: Tranzitlar uchun alohida hisob-kitob
    const tranzitRoutes = ['O\'ty-AJ', 'Tranzit 317/70', 'O\'ty-UJ', 'Tranzit'];
    const tranzitData: Record<string, number> = {};
    let hasTranzit = false;
    
    // Initialize moveTypeData for all moveTypes
    moveTypes.forEach(mt => {
      moveTypeData[mt] = {};
      ERJU_DATA.forEach(erju => {
        moveTypeData[mt][erju.name] = 0;
      });
      moveTypeTotals[mt] = 0;
    });
    
    // Initialize tranzit data
    tranzitRoutes.forEach(route => {
      tranzitData[route] = 0;
    });
    ...
    rows.forEach(r => {
      ...
      if (moveTypeData[move]) {
        moveTypeData[move][erjuName] = (moveTypeData[move][erjuName] || 0) + fuel;
        moveTypeTotals[move] += fuel;
      }
      
      // Check for tranzit routes in the route field
      const routeField = (r.route || '').trim();
      tranzitRoutes.forEach(tranzitRoute => {
        const routeLower = routeField.toLowerCase();
        const tranzitLower = tranzitRoute.toLowerCase();
        if (routeLower.includes(tranzitLower)) {
          tranzitData[tranzitRoute] += fuel;
          hasTranzit = true;
        }
      });
    });
    
    // Build the move type summary table HTML
    const erjuShortNames = ERJU_DATA.map(e => e.short);
    const moveTypeSummaryRows = moveTypes.map(mt => {
      const erjuCells = ERJU_DATA.map(erju => {
        const val = moveTypeData[mt][erju.name] || 0;
      ...
      return `
        <tr>
          <td class="left">${mt}</td>
          ${erjuCells}
          <td class="num">${fmt(moveTypeTotals[mt])}</td>
        </tr>`;
    }).join('');
    
    // Grand total row for move type summary
    const moveTypeGrandCells = ERJU_DATA.map(erju => {
      let total = 0;
      moveTypes.forEach(mt => {
        total += moveTypeData[mt][erju.name] || 0;
      });
      return `<td class="num grand-num">${fmt(total)}</td>`;
    }).join('');
    
    const moveTypeGrandTotal = Object.values(moveTypeTotals).reduce((a, b) => a + b, 0);
    const moveTypeGrandRow = `
      <tr class="grand-r">
        <td class="left" style="font-weight:800">Jami</td>
        ${moveTypeGrandCells}
        <td class="num grand-num">${fmt(moveTypeGrandTotal)}</td>
      </tr>`;
    
    // Tranzit table HTML (if there's any tranzit data)
    let tranzitTableHtml = '';
    if (hasTranzit || Object.values(tranzitData).some(v => v > 0)) {
      const tranzitRows = Object.entries(tranzitData)
        .filter(([_, val]) => val > 0)
        .map(([route, val]) => `
          <tr>
      ...
      if (tranzitRows) {
        tranzitTableHtml = `
          <div class="summary-title" style="margin-top:15px;">Tranzit yo'nalishlari bo'yicha</div>
          <table class="sum-table" style="width:50%;">
            <thead>
              <tr>
                <th class="left">Yo'nalish</th>
                <th>Jami</th>
              </tr>
            </thead>
            <tbody>
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          ...
          <!-- HARAKAT TURI BO'YICHA HISOBLASH -->
          <div class="summary-title">Harakat turi bo'yicha hisob-kitob</div>
          <table class="sum-table">
            <thead>
              <tr>
                <th class="left">Harakat turi</th>
                ${ERJU_DATA.map(e => `<th>${e.short}</th>`).join('')}
                <th>Jamit</th>
              </tr>
            </thead>
            <tbody>
          ...
          <div class="stat-line" style="margin-top:15px;">Jami teplovozlar soni: ${totalLocos} ta &nbsp;|&nbsp; Jami yozuvlar: ${rows.length} ta</div>
        </body>
      </html>
    `);
        ...
        <section className="bg-white/[0.02] border border-white/10 rounded-[2rem] overflow-hidden shadow-2xl">
          <div className="overflow-x-hidden">
            <table className="w-full text-left table-auto">
              <thead className="bg-white/5 text-[11px] uppercase text-blue-300 font-bold tracking-wider">
                <tr>
              ...
              <tbody className="divide-y divide-white/5">
                {rows.map((row, idx) => (
                  <tr key={row.id} className="hover:bg-white/5 transition-colors group">
                    <td className="p-3 text-slate-500 font-mono text-center text-sm">{rows.length - idx}</td>
                    <td className="p-3 font-mono text-sm">
                      <span className="text-slate-200">{row.time}</span>
                    </td>
                    <td className="p-3"><span className="text-blue-400 font-bold text-sm">{row.locoSeries}</span></td>
                    <td className="p-3 text-sm text-slate-300">{row.locoCode}</td>
                    <td className="p-3 text-sm text-slate-300">{row.trainIndex}</td>
                    <td className="p-3 text-sm text-slate-300">{row.supplyPoint}</td>
                    <td className="p-3 text-sm">{row.moveType}</td>
                    <td className="p-3 text-sm text-slate-300">{row.locoNumber}</td>
                    <td className="p-3 text-sm text-slate-300">{row.trainNumber}</td>
                    <td className="p-3 text-sm text-slate-300">{row.weight}</td>
                    <td className="p-3 text-sm text-slate-300">{row.staffName}</td>
                    <td className="p-3 text-center text-amber-500 font-mono text-sm">{Number(row.balanceBefore).toLocaleString()}</td>
                    <td className="p-3 text-center text-emerald-400 font-bold font-mono text-sm">{Number(row.fuelAmount).toLocaleString()}</td>
                    <td className="p-3 text-center font-bold font-mono text-sm">{(Number(row.balanceBefore) + Number(row.fuelAmount)).toLocaleString()}</td>
